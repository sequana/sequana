...
def plot_venn(subsets, labels=None, title=None, ax=None, alpha=0.8, weighted=False, colors=("r", "b", "y")):
    ...
    vf = venn_function(subsets, set_labels=labels, ax=ax, alpha=alpha, set_colors=colors)
    ...
    for center, radius in zip(vf.centers, vf.radii):
        circle = pylab.Circle(center, radius=radius, linestyle="-", edgecolor="k", lw=2, facecolor="none", alpha=1)
        ax.add_patch(circle)
    if title:
        pylab.title(title)
    return vf
