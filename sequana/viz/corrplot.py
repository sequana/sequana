...
class Corrplot(Linkage):
    ...
    def __init__(self, data, na=0, compute_correlation=False):
        ...
        self.params = {"colorbar.N": 100, "colorbar.shrink": 0.8, "colorbar.orientation": "vertical"}
    ...
    def plot(
        ...
        ax.set_xticklabels(labels, rotation=rotation, color=label_color, fontsize=fontsize, ha="left")
        ...
        if grid is not False:
            ...
            for i in range(0, width):
                ...
                if mode == "lower":
                    plt.axvline(i + 0.5, ymin=1 - ratio1, ymax=0.0, color=grid)
                    plt.axhline(i + 0.5, xmin=0, xmax=ratio2, color=grid)
                if mode == "upper":
                    plt.axvline(i + 0.5, ymin=1 - ratio2, ymax=1, color=grid)
                    plt.axhline(i + 0.5, xmin=ratio1, xmax=1, color=grid)
                if mode in ["method", "both"]:
                    plt.axvline(i + 0.5, color=grid)
                    plt.axhline(i + 0.5, color=grid)

            # can probably be simplified
            if mode == "lower":
                plt.axvline(-0.5, ymin=0, ymax=1, color="grey")
                plt.axvline(width - 0.5, ymin=0, ymax=1.0 / width, color="grey", lw=2)
                plt.axhline(width - 0.5, xmin=0, xmax=1, color="grey", lw=2)
                plt.axhline(-0.5, xmin=0, xmax=1.0 / width, color="grey", lw=2)
                plt.xticks([])
                for i in range(0, width):
                    plt.text(
                        i,
            ...
            # can probably be simplified
                ...
                plt.axvline(width - 0.5, ymin=0, ymax=1, color="grey", lw=2)
                plt.axvline(-0.5, ymin=1 - 1.0 / width, ymax=1, color="grey", lw=2)
                plt.axhline(-0.5, xmin=0, xmax=1, color="grey", lw=2)
                plt.axhline(width - 0.5, xmin=1 - 1.0 / width, xmax=1, color="grey", lw=2)
                plt.yticks([])
                for i in range(0, width):
                    plt.text(
                        -0.6 + i,
            ...
            for y in range(height):
                ...
                if method in ["ellipse", "square", "rectangle", "color"]:
                    if method == "ellipse":
                        func = Ellipse
                        patch = func(
                            (x, y), width=1 * self.shrink, height=(self.shrink - d_abs * self.shrink), angle=rotate
                        )
                    else:
                        func = Rectangle
                        ...
                        if method == "color":
                            ...
                            h = 1
                            offset = 0
                        patch = func((x + offset - 0.5, y + offset - 0.5), width=w, height=h, angle=0)
                    if self.edgecolor:
                    ...
                    patch = Circle((x, y), radius=d_abs * self.shrink / 2.0)
                    if self.edgecolor:
                        patch.set_edgecolor(self.edgecolor)
                    # patch.set_facecolor(cmap(d_abs))
                    colors.append(datum)
                    ...
                    ax.text(
                        ...
                        weight="bold",
                        alpha=min(1, max(0.5, d_abs)),
                    )
                    # withdash=False)
                elif method == "pie":
                    S = 360 * d_abs
                    patch = [
                        Wedge((x, y), 1 * self.shrink / 2.0, -90, S - 90),
                        Wedge((x, y), 1 * self.shrink / 2.0, S - 90, 360 - 90),
                    ]
