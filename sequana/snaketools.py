...
class SnakeMakeStats(object):
    ...
    def plot(self, fontsize=16):
        ...
        # sort by time (total should be first on the top)
    ...
    def plot_and_save(self, filename="snakemake_stats.png", outputdir="report"):
        import pylab
        ...
        if data is None:
            ...
            if os.path.exists(data):
                if data.endswith(".yaml") or data.endswith(".yml"):
                    ...
                    with open(data, "r") as fh:
                        try:
                            self._yaml_code = yaml.load(json.dumps(json.loads(fh.read())), Loader=yaml.FullLoader)
                        except Exception:
                            # olf python version
        ...
        if "requirements" in self._yaml_code.keys():
            for requirement in self._yaml_code["requirements"]:
                if os.path.exists(requirement):
                    ...
                    try:
                        logger.info(f"Copying {requirement} from sequana")
                        shutil.copy(sequana_data(requirement), target)
                    except Exception:
                        logger.warning(f"This requirement {requirement} was not found in sequana.")
...
def init(filename, namespace, mode="error"):
    ...
    """
    logger.warning(
        "Message for developers. This will be deprecated soon. " "Please use the PipelineManager instead setup() method"
    )
