...
def test_canvasjs_linegraph():
    bed = bedtools.SequanaCoverage(
        f"{test_dir}/data/bed/JB409847.bed", reference_file=f"{test_dir}/data/fasta/JB409847.fasta"
    )
    ...
    df = bed[0].df
    csv = df.to_csv(columns=["pos", "cov", "gc"], index=False, float_format="%.3g")
    ...
    # set legend
    cjs.set_legend({"verticalAlign": "bottom", "horizontalAlign": "center", "cursor": "pointer"}, hide_on_click=True)
    # set axis
    cjs.set_axis_x({"title": "Position (bp)", "labelAngle": 30, "minimum": 0, "maximum": len(df)})
