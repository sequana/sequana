...
def _test_fasta_gff_correction2(tmpdir):
    ...
    assert f.get_all_start_codons("test.fa", "test.gff3", "AE000666_1", strand="+") == defaultdict(
        int, {"ATG": 580, "TTG": 143, "GTG": 214}
    )

    assert f.get_all_stop_codons("test.fa", "test.gff3", "AE000666_1", strand="+") == defaultdict(
        int, {"TGA": 443, "TAA": 272, "TAG": 222}
    )

    assert f.get_all_start_codons("test.fa", "test.gff3", "AE000666_1", strand="-") == defaultdict(
        int, {"CAT": 594, "CAC": 206, "CAA": 132}
    )

    assert f.get_all_stop_codons("test.fa", "test.gff3", "AE000666_1", strand="-") == defaultdict(
        int, {"TTA": 285, "TCA": 440, "CTA": 207}
    )
